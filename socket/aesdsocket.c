...
#define BUTTON_PLAY_NEXT "22"
#define BUTTON_PLAY_PREVIOUS "23"
...
int main(int argc, char *argv[])
{
	...
	if(export_gpio(BUTTON_PAUSE) == -1){
		perror("Export failed for pause gpio pin");
		return -1;
	}
	if(export_gpio(BUTTON_PLAY) == -1){
		perror("Export failed for play gpio pin");
		return -1;
	}
	if(export_gpio(BUTTON_PLAY_NEXT) == -1){
		perror("Export failed for play next gpio pin");
		return -1;
	}
	if(export_gpio(BUTTON_PLAY_PREVIOUS) == -1){
		perror("Export failed for play previous gpio pin");
		return -1;
	}
